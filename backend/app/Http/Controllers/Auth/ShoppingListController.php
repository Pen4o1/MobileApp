...
class ShoppingListController extends Controller
{
    public function getShoppingPlan()
    {
        ...
        if(!$mealPlan)
        {
            return response()->json(['no meal plan found'], 404);            
        }
        ...
        foreach ($mealPlan as $meal) {
            if (isset($meal['recipe_ingredients']['ingredient'])) {
                // if a single recipe
                foreach ($meal['recipe_ingredients']['ingredient'] as $ingredient) {
                    $shoppingList[] = $ingredient;
                }
            } elseif (isset($meal['combined_recipes'])) {
                // if there are combined recipes
                foreach ($meal['combined_recipes'] as $recipe) {
                    if (isset($recipe['recipe_ingredients']['ingredient'])) {
                        foreach ($recipe['recipe_ingredients']['ingredient'] as $ingredient) {
                            $shoppingList[] = $ingredient;
                        }
                    }
                }
    ...
    /*
    THIS IS A TEST this is the solution that im working on
    ...
    class ShoppingListController extends Controller
    {
        public function getShoppingPlan()
        {
            ...
            $combinedIngredients = $this->combinedIngredients($shoppingList);
        ...
        private function combinedIngredients(array $ingredientList)
        {
            $aggregated = [];
            ...
            foreach ($ingredientList as $ingredient) {
                ...
                if ($aggregated[$name]['unit'] === $unit || !$unit) {
                    ...
                    // Handle unit conversion if necessary (e.g., tsp to tbsp)
        ...
        private function convertUnit($quantity, $fromUnit, $toUnit)
        {
            ...
            return $quantity; // If no conversion is possible, return the original quantity
        }
    }
    */
}
